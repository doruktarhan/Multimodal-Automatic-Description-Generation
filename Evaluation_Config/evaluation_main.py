...
from Evaluation_Config.vertex_evaluation import EvalDatasetVertex, EvalTask
from Evaluation_Config.config_manager import ConfigManager
from Evaluation_Config.metrics_registry import MetricsRegistry, MetricFactory
from vertexai.evaluation import MetricPromptTemplateExamples
import vertexai.preview.evaluation
...
config_manager = ConfigManager("Evaluation_Config/evaluation_config.yaml")
test_data_path = config_manager.get_directory("test_data_path")
test_data = load_json(test_data_path)
# Base directory containing model folders (up to the LLM_7B part)
base_dir = config_manager.get_directory("base_dir")
...
for metric_config in config_manager.get_metrics_config():
    metric = MetricFactory.create_from_config(metric_config)
    metrics_registry.register(metric_config["name"], metric)

# Load built-in metrics from configuration
for bim_config in config_manager.get_built_in_metrics_config():
    metric_name = bim_config["name"]
    template_key = bim_config["template_key"]
    try:
        # Access the template from MetricPromptTemplateExamples.Pointwise
        metric_template = getattr(MetricPromptTemplateExamples.Pointwise, template_key)
        metrics_registry.register(metric_name, metric_template)
        print(f"Successfully registered built-in metric: {metric_name}")
    except AttributeError:
        print(f"Error: Built-in metric template '{template_key}' not found in MetricPromptTemplateExamples.Pointwise.")
    except Exception as e:
        print(f"Error registering built-in metric '{metric_name}': {e}")
...
if not model_dirs:
    ...
    for model_dir in model_dirs:
        ...
        try:
            ...
            eval_task = EvalTask(
                dataset = eval_ds,
                metrics = [i for i in metrics_registry.get_all_metrics()],
                experiment = experiment_name
                )
            
            # Run evaluation
            logging.info(f"Starting evaluation for experiment: {experiment_name}")
