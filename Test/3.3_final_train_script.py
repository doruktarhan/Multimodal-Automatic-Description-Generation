...
def main():
    ...
    wandb.init(
        ...
        name=f"{selected_model}-training-test-{stream_chunk_size}-{micro_batch_size}-{learning_rate}-Lora_{use_lora}",
        config={
        ...
        for epoch in range(epochs_per_chunk):
            ...
            for step, batch in enumerate(train_loader):
                ...
                try:
                    ...
                    if seq_len > 4096:  # Adjusted safety limit for GPU
                        print(f"Warning: Sequence length {seq_len} is very long, might cause memory issues")
                    
                    
                    # Forward pass - time it for benchmarking
                    ...
                    # Backward pass - time it for benchmarking
                    ...
                    # # Force memory cleanup / enable this if the memory climbs up steadily 
                    # gc.collect()
                    # if torch.cuda.is_available():
